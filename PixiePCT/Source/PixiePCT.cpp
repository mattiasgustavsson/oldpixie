...
void PixiePCTMain(const char* commandLine)
	...
	for (int i=0; i<cmd.GetCommandLineFlagCount(); i++)
		{
		const char* flag = cmd.GetCommandLineFlag(i).GetString();
		if (StrNICmp(flag,"colorkey=",9)==0)
			{
			const char* color = flag + 9;
			colorKey = strtol(color,0,16);
			colorKey |= 0xff000000;
			useColorKey = true;
			}
		}
	...
	for (int i=1; i<cmd.GetCommandLineStringCount(); i++)
		...
		if (file->Exists())
			{
			LoadFile(filename,images,fonts,!strip,dither, useColorKey, colorKey);
			}
		delete file;
		...
		if (directory->Exists())
			...
			LoadDirectory(filename,images,fonts,!strip,dither, useColorKey, colorKey);
			}
		delete directory;
		}
...
void LoadFile(const char* filename,	Array<ImageFile>& images, Array<FontFile>& fonts, bool convertImmediately, bool dither, bool useColorKey, unsigned int colorKey)
	{
...
void LoadDirectory(const char* filename, Array<ImageFile>& images, Array<FontFile>& fonts, bool convertImmediately, bool dither, bool useColorKey, unsigned int colorKey)
	{
	Platform_FileSystem_Directory* directory=Platform::GetPlatform_FileSystem()->CreateDirectoryObject(filename);
	for (int i=0; i<directory->GetSubdirectoryCount(); i++)
		...
		LoadDirectory(buffer,images,fonts,convertImmediately,dither,useColorKey,colorKey);
		}
	...
	for (int i=0; i<directory->GetFileCount(); i++)
		...
		LoadFile(buffer,images,fonts,convertImmediately,dither,useColorKey,colorKey);
		}
	}