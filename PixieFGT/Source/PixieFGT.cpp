...
void GetKerning(const char* fontname, int fontsize, bool antialias, DynamicArray<Kerning>& kerning)
	{
	if (antialias)
		{
		fontsize*=16;
...
void CreateStrip(const char* filename, DynamicArray<struct Glyph*> glyphs, int spacing)
	{
...
void CreateTexture(const char* filename, DynamicArray<struct Glyph*> glyphs, int spacing)
	{
	// Calulate total size
	int size=0;
	for (int i=0; i<glyphs.GetItemCount(); i++)
