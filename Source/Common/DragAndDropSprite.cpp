...
bool DragAndDropSprite::OnMouseOver(int x, int y, bool button, StringId& eventId, void*& userData, bool forcehit)
	{
	if (!IsEnabled())
		{
		dragging_=false;
	...
	if (bitmapStrip.GetCelCount()!=0)
		...
		if (bitmap.GetPixelAlpha(x-(int)(GetX()+GetOriginX()),y-(int)(GetY()+GetOriginY()))>GetAlphaThreshold())
			{
			if (IsEnabled())
				{
			//	highlighted_=true;
