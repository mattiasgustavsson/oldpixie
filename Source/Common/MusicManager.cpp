	...
	switch (state_)
		...
		case State_FadingIn:
			...
			if (currentTime_>fadeInTime_)
				{
				currentMusic_->SetVolume(currentMusicVolume_*masterVolume_);
				state_=State_Playing;
				return;
				}

			currentMusic_->SetVolume(currentMusicVolume_*(currentTime_/fadeInTime_)*masterVolume_);
			} break;
		...
		case State_FadingOut:
			...
			currentMusic_->SetVolume(currentMusicVolume_*(1.0f-(currentTime_/fadeOutTime_))*masterVolume_);
			} break;
		...
		case State_CrossFading:
			...
			currentMusic_->SetVolume(currentMusicVolume_*(currentTime_/crossFadeTime_)*masterVolume_);
			switchMusic_->SetVolume(currentMusicVolume_*(1.0f-(currentTime_/crossFadeTime_))*masterVolume_);
			} break;
		}
	}
...
void MusicManager::PlayMusic(const Filename& filename, bool loop, float volume, float fadeInTime)
	...
	if (fadeInTime==0)
		{
		state_=State_Playing;
		currentMusic_=new Music( *siOldResourceManager->GetAsset( filename.GetStringId() ) );
		currentMusic_->SetVolume(volume*masterVolume_);
