...
OldResourceManager::~OldResourceManager()
	...
	// Delete all asset
	HashTableIterator<HashTableKey_Pointer,AssetEntry*> itM(asset_);
	while(itM.IsValid())
		{
		AssetEntry* assetEntry=itM.GetCurrent();
		delete assetEntry->asset;
		delete assetEntry->staticBuffer;
		delete[] assetEntry->buf;
		delete assetEntry;
		itM.MoveNext();
		}
	}
...
OldResourceManager::AssetEntry* OldResourceManager::GetAssetEntry(const Asset* asset)
	{
	HashTableIterator<HashTableKey_Pointer,AssetEntry*> it(asset_);
	if (it.Find(HashTableKey_Pointer(asset)))
		{
		return it.GetCurrent();
		}
	...
	entry->asset=asset;
	entry->referenceCount=0;
	asset_.Insert(HashTableKey_Pointer(asset),entry);
	return entry;
	}
...
void OldResourceManager::IncreaseReferenceCount(const Asset* asset)
	{	
	AssetEntry* entry=GetAssetEntry(asset);
	entry->referenceCount++;
	}
...
void OldResourceManager::DecreaseReferenceCount(const Asset* asset)
	{
	AssetEntry* entry=GetAssetEntry(asset);
	Assert(entry->referenceCount>=1,"Tried to decrease reference count when already 0");
	if (entry->referenceCount<=0)
	...
	if (entry->referenceCount==0)
		{
		asset_.Remove(HashTableKey_Pointer(asset));
		delete entry->asset;
		delete entry->staticBuffer;
		delete[] entry->buf;
		delete entry;
		}
	}
...
const Asset* OldResourceManager::GetAsset(StringId filename)
	{
	if (!filename.GetString())
		{
		return 0;
	...
	while(it.IsValid())
		{
		AssetEntry* entry=it.GetCurrent();
		if (entry->filename==filename && entry->asset)
			{
			return entry->asset;
			}
		it.MoveNext();
	...
	int size = loadAsset.GetSize();
	char* buf = new char[ size ]; /* TODO: Don't leak this */
	if( loadAsset.Open() )
		{
		loadAsset.Read( buf, size );
		loadAsset.Close();
		}
	else
		{
	...
	StaticBuffer* membuf = new StaticBuffer( buf, size ); /* TODO: Don't leak this */
	
	const Asset* asset = new Asset(membuf);
	
	AssetEntry* entry=GetAssetEntry(asset);
	entry->filename=filename;
	entry->buf = buf;
	entry->staticBuffer = membuf;
	return entry->asset;
	}
...
void OldResourceManager::LoadAsset(const Filename& filename)
	{
	const Asset* asset=GetAsset(filename.GetStringId());
	IncreaseReferenceCount(asset);
	}


//*** UnloadAsset ***

void OldResourceManager::UnloadAsset(const Filename& filename)
	{
	const Asset* asset=GetAsset(filename.GetStringId());
	DecreaseReferenceCount(asset);
	}
