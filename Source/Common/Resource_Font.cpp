...
void Resource_Font::BlitWrap(Bitmap& bitmap, int x, int y, const char* text, int limit, int width, int hspacing, int vspacing, unsigned short modulate, unsigned char alpha) const
	{
	if (!font_)
		{
		return;
	...
	font_->BlitWrap(bitmap,x,y,text,limit,width,hspacing,vspacing,modulate,alpha);
	}

void Resource_Font::BlitCentered(Bitmap& bitmap, int x, int y, const char* text, int limit, int width, int hspacing, int vspacing, unsigned short modulate, unsigned char alpha) const
	{
	if (!font_)
		{
		return;
	...
	font_->BlitCentered(bitmap,x,y,text,limit,width,hspacing,vspacing,modulate,alpha);
	}