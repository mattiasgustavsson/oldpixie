...
void Button::Render(Bitmap& bitmap)
	...
	if( pressOverride_ > 0 )
		{
		--pressOverride_;
		if( IsEnabled() ) SetState(State_Pressed);
		}
	else
		{
...
bool Button::OnMouseOver(int x, int y, bool button, StringId& eventId, void*& userData, bool forcehit)
	...
	if( usingBoundingBox_ )
		...
		hovering =  ( alpha > GetAlphaThreshold() );
		}
...
void Button::SimulatePressed( int render_count )
	{
	pressOverride_ = render_count;
	}