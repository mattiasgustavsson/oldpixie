...
void Music::Update()
	...
	if (lastCopyReachedEnd_)
		...
		if( !didWrap_ )
			{
			if( streamPosition < Music_BufferSize / 2 )
				{
				didWrap_ = true;
				}
			}
		int lastCopyEnd = lastCopyDestinationStart_ + lastCopyByteCount_;
		if( lastCopyDestinationStart_ > 0 )
			{
			if( lastCopyEnd > Music_BufferSize )
				{
				lastCopyEnd -= Music_BufferSize;
				}
			if( streamPosition >= lastCopyEnd )
				{
				Stop();
				}
			}
			...
			{
			if( didWrap_ && streamPosition >= lastCopyEnd )
				{
				Stop();
				}
...
void Music::CopySoundChunk(int bufferOffset,int maxBytes)
	...
	if (looping_)
		...
		while (lastCopyByteCount_<maxBytes)
			{
			int count=format_->CopySoundChunk(streamBuffer_+lastCopyByteCount_,maxBytes-lastCopyByteCount_, looping_ );
			if (count<maxBytes)
				{
		...
		{
		lastCopyByteCount_=format_->CopySoundChunk(streamBuffer_,maxBytes, looping_);
