...
bool Font::WillWordFit(int xp, int maxX, const char* text, int hspacing) const
	...
	int i=0;
	while (text[i]!=0 && !(text[i]>2 && text[i]<=' '))
		{
		const Character& character=GetCharacter(text[i]);
		int kerning=0;
		if (i>0)
			{
			kerning=GetKerning(text[i-1],text[i]);
			}
		xp+=character.spacing+hspacing +kerning;
		if (xp>=maxX)
			{
			return false;
			}
...
int Font::FindLineLength( int xp, int maxX, const char* text, int hspacing ) const
	{
	if (size_==0)
		{
		return 0;
	...
	while( *text != 0 )
		...
		if (*text>2 && *text<=' ')
			{
			if (!WillWordFit(xp,maxX,text+1,hspacing))
				{
				return length;
				}
			}

		const Character& character=GetCharacter(*text);
		xp+=character.spacing+hspacing;
		length+=character.spacing+hspacing;
		text++;
		}
	return length;
	}
...
void Font::BlitWrap(Bitmap& bitmap, int x, int y, const char* text, int limit, int width, int hspacing, int vspacing, unsigned short modulate, unsigned char alpha) const
	{
	if (size_==0)
		{
		return;
...
void Font::BlitCentered(Bitmap& bitmap, int x, int y, const char* text, int limit, int width, int hspacing, int vspacing, unsigned short modulate, unsigned char alpha) const
	{
	if (size_==0)
		{
		return;
	...
	for (int i=0; i<layerCount_; i++)
		{	
		int lim = limit;
		int lineWidth = FindLineLength( x, x+width, text, hspacing );
		int xp=x+(width-lineWidth)/2;
		int yp=y;
		for (int j=0; j<textLength; j++)
			{	
			if (text[j]=='\n' || xp>=x+width)
				{
				lineWidth = FindLineLength( x, x+width, &text[j+(text[j]=='\n' ? 1: 0)], hspacing );
				xp=x+(width-lineWidth)/2;
				yp+=size_+vspacing;
				continue;
				}
			...
			if (text[j]>2 && text[j]<=' ')
				...
				if (!WillWordFit(xp,x+width,&text[j+1],hspacing))
					{
					lineWidth = FindLineLength( x, x+width, &text[j+1], hspacing );
					xp=x+(width-lineWidth)/2;
					yp+=size_+vspacing;
					continue;
					}
