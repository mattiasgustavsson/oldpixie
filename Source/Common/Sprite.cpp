...
bool Sprite::PickPixel(int x, int y) const
	...
	if (GetBitmap().GetCelCount()!=0)
		...
		if (bitmap.GetPixelAlpha(x-(int)(GetX()-GetOriginX()),y-(int)(GetY()-GetOriginY()))>alphaThreshold_)
			{
			return true;
			}
		}
...
void Sprite::SetAlphaThreshold( int alpha )
	{
	alphaThreshold_ = alpha;
	}

int Sprite::GetAlphaThreshold() const
	{
