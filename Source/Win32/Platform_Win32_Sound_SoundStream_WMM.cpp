...
void CALLBACK Platform_Win32_Sound_SoundStream_WMM::waveOutProc(HWAVEOUT hWaveOut, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2)
    ...
    Platform_Win32_Sound_SoundStream_WMM* wmm=(reinterpret_cast<Platform_Win32_Sound_SoundStream_WMM*>(dwInstance));
	...
	EnterCriticalSection(&wmm->waveCriticalSection_);
    wmm->firstUsedWaveBlock_++;
	wmm->firstUsedWaveBlock_%=WaveBlockCount;
    LeaveCriticalSection(&wmm->waveCriticalSection_);
	}
...
Platform_Win32_Sound_SoundStream_WMM::Platform_Win32_Sound_SoundStream_WMM(Platform_Win32_Sound_WMM* manager, int channels, int frequency, int bitsPerSample, int size):
    ...
    // more sound cards).   
    MMRESULT result=waveOutOpen(&hWaveOut_, WAVE_MAPPER, &format, (DWORD_PTR)waveOutProc, (DWORD_PTR)this, CALLBACK_FUNCTION);
