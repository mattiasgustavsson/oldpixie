...
BOOL CALLBACK Platform_Win32_Screen::MonitorEnumProc( HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData )
	{
	Platform_Win32_Screen* screen = (Platform_Win32_Screen*)dwData;

	if( screen->monitorEnumCount_ == screen->monitor_ )
		{
		screen->monitorLeft_ = lprcMonitor->left;
		screen->monitorTop_ = lprcMonitor->top;
		screen->monitorRight_ = lprcMonitor->right;
		screen->monitorBottom_ = lprcMonitor->bottom;
		return FALSE;
		}

	++screen->monitorEnumCount_;
	return TRUE;
	}
...
Platform_Win32_Screen::Platform_Win32_Screen(Platform_Win32_OS* os):
	...
	if (os->GetCommandLineString())
		...
		while (token)
			...
			if (strnicmp(token,"-monitor", 8 )==0)
				{
				monitor_ = atoi( token + 8);
				}
	...
	if (result)
		{
		monitorLeft_ = desktopRect.left;
		monitorTop_ = desktopRect.top;
		monitorRight_ = desktopRect.right;
		monitorBottom_ = desktopRect.bottom;
		}

	monitorEnumCount_ = 0;
	EnumDisplayMonitors( NULL, NULL, MonitorEnumProc, (long) this );

	fullscreenWidth_=monitorRight_-monitorLeft_;
	fullscreenHeight_=monitorBottom_-monitorTop_;
	...
	windowedX_/=2;
	windowedY_/=2;
	windowedX_+=monitorLeft_;
	windowedY_+=monitorTop_;
	}
...
void Platform_Win32_Screen::FirstTimeInitialize()
	...
	else if ( crtMode_ && !forcedx9_ )
		{
		SetTechnology(Technology_D3D9_CRT); 
		}	
	else
		{
...
void Platform_Win32_Screen::Present(unsigned short* bitmapData, int bitmapWidth, int bitmapHeight, unsigned short modulate, unsigned short backgroundColor)
	...
	if (showfps_ && Platform::GetPlatform_Time())
		...
		DebugText(bitmapData,bitmapWidth,bitmapHeight,16,26,fps,0);
		DebugText(bitmapData,bitmapWidth,bitmapHeight,15,25,fps,color);
...
void Platform_Win32_Screen::SetTechnology(Platform_Win32_Screen::Technology technology)
	...
	switch(technology_)
		{
// 		case Technology_D3D9_CRT:
// 			{
// 			technologyInstance_=new Platform_Win32_Screen_D3D9_CRT(windowHandle_, GetFullscreen(), GetX(), GetY(), GetWidth(), GetHeight(), GetInterpolationMode(), monitor_);
// 			} break;
// 
		case Technology_D3D9:
			{
			technologyInstance_=new Platform_Win32_Screen_D3D9(windowHandle_, GetFullscreen(), GetX(), GetY(), GetWidth(), GetHeight(), GetInterpolationMode(), monitor_);
			} break;

		case Technology_DDraw:
			{
			technologyInstance_=new Platform_Win32_Screen_DDraw(windowHandle_, GetFullscreen(), GetX(), GetY(), GetWidth(), GetHeight(), monitor_);
			technologyInstance_->SetInterpolationMode(GetInterpolationMode());
			} break;

		case Technology_GDI:
			{
			technologyInstance_=new Platform_Win32_Screen_GDI(windowHandle_, GetFullscreen(), GetX(), GetY(), GetWidth(), GetHeight(), monitor_);
			technologyInstance_->SetInterpolationMode(GetInterpolationMode());
			} break;
		}
...
void Platform_Win32_Screen::SetCrtMode(bool enabled)
	{
	if (crtMode_==enabled)
		{
		return;
	...
	if (technologyInstance_)
		...
		SetTechnology(enabled && !forcedx9_ ? Technology_D3D9_CRT : Technology_D3D9 );
...
bool Platform_Win32_Screen::GetCrtMode()
	{
	return crtMode_;
	}
...
void Platform_Win32_Screen::SetFullscreen(bool fullscreen)
	...
	if (technologyInstance_)
		...
		if (!GetFullscreen())
			{
			MoveWindow(windowHandle_,windowedX_,windowedY_,windowedWidth_,windowedHeight_,TRUE);
			SetWindowSize();
			}
...
int Platform_Win32_Screen::GetX()
	{
	if (fullscreen_)
		{
		return monitorLeft_;
...
int Platform_Win32_Screen::GetY()
	{
	if (fullscreen_)
		{
		return monitorTop_;
